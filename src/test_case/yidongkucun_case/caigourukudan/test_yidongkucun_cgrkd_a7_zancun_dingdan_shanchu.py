from src.pages.yidongkucun.caigourukudan import yidongkucun_a3_sousuo_yishenhe, yidongkucun_a2_search, \
    ...
    yidongkucun_cgrkd_a6_zancun_dingdan_tijiao,yidongkucun_cgrkd_a7_zancun_dingdan_shanchu
...
class TestLong7(unittest.TestCase):
    def setUp(self):
    ...
    def test_case_7(self):
        ...
        #引入shanchu的页面元素
        self.shanchu = yidongkucun_cgrkd_a7_zancun_dingdan_shanchu.shanchu_page(self.driver)
        #点击提交按钮
        self.shanchu.click_cgrk_shouye_zancun_shanchu()
        self.assertEqual(self.shanchu.dy_cgrk_shouye_zancun_shanchu, 1, '断言失败，删除按钮点击失败')
        ...
        if self.login.dy_no_cgrk_yingyong_zancuntubiao == 1:
            print("test_case7:pass")
        #如果查询不成功，判断为失败
        else:
            self.assertEqual(self.login.dy_no_cgrk_yingyong_zancuntubiao, 1, '断言失败，test_case_7执行失败！')
