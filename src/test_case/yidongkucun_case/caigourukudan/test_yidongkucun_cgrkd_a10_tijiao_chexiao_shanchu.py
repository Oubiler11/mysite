from src.pages.yidongkucun.caigourukudan import yidongkucun_a3_sousuo_yishenhe, yidongkucun_a2_search, \
    ...
    yidongkucun_cgrkd_a9_tijiao_dingdan_chexiao,yidongkucun_cgrkd_a10_tijiao_chexiao_shanchu
...
class TestLong10(unittest.TestCase):
    def setUp(self):
    ...
    def test_case_10(self):
        ...
        #引入shan的页面元素
        self.shan= yidongkucun_cgrkd_a10_tijiao_chexiao_shanchu.shanchu_page(self.driver)

        #筛选后的订单，进入订单详情页，点击删除
        self.shan.click_cgrk_shouye_tijiao_chexiao_shanchu()
        self.assertEqual(self.shan.dy_cgrk_shouye_tijiao_chexiao_shanchu, 1, '删除失败，删除点击失败')

        #筛选后的订单，进入订单详情页，确认删除
        self.shan.click_cgrk_shouye_tijiao_queren_shanchu()
        self.assertEqual(self.shan.dy_cgrk_shouye_tijiao_queren_shanchu, 1, '确认删除失败，删除点击失败')
        ...
        if self.xiao.dy_cgrk_shouye_tijiao_yitijiao == 1:
            print("test_case10:pass")
        #如果查询不成功，判断为失败
        else:
            self.assertEqual(self.xiao.dy_cgrk_shouye_tijiao_yitijiao, 1, '断言失败，test_case_10执行失败！')
