...
class shanchu_page(Base_page.Base_page):
    ...
    # 提交订单撤回后删除
    cgrk_shouye_tijiao_chexiao_shanchu = (by.XPATH, "//*[@class='android.view.View'][@content-desc='删除']")


    def click_cgrk_shouye_tijiao_chexiao_shanchu(self):
        # 判断能否进行点击操作，如果能够点击操作，赋值1，同时输出点击成功的提示
        try:
            self.find_element (*self.cgrk_shouye_tijiao_chexiao_shanchu).click ()
            self.dy_cgrk_shouye_tijiao_chexiao_shanchu = 1
            print ("删除点击成功")
        # 如果不能进行点击操作，赋值2，同时输出点击不成功的提示，并且输出当前页的截图
        except:
            self.dy_cgrk_shouye_tijiao_chexiao_shanchu = 2
            print ("删除点击失败，输出当前截图")
            self.driver.get_screenshot_as_file (
                "C:\\Users\\Administrator\\PycharmProjects\\pythonProject\\cqydtest\\src\\image\\click_cgrk_shouye_tijiao_chexiao_shanchu.png")

    #确认订单撤回后删除
    cgrk_shouye_tijiao_queren_shanchu = (by.XPATH, "//*[@class='android.view.View'][@content-desc='确定']")

    def click_cgrk_shouye_tijiao_queren_shanchu(self):
        # 判断能否进行点击操作，如果能够点击操作，赋值1，同时输出点击成功的提示
        try:
            self.find_element (*self.cgrk_shouye_tijiao_queren_shanchu).click ()
            self.dy_cgrk_shouye_tijiao_queren_shanchu = 1
            print ("删除点击成功")
        # 如果不能进行点击操作，赋值2，同时输出点击不成功的提示，并且输出当前页的截图
        except:
            self.dy_cgrk_shouye_tijiao_queren_shanchu = 2
            print ("删除点击失败，输出当前截图")
            self.driver.get_screenshot_as_file (
                "C:\\Users\\Administrator\\PycharmProjects\\pythonProject\\cqydtest\\src\\image\\click_cgrk_shouye_tijiao_chexiao_shanchu.png")