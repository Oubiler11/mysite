...
class cangku_page(Base_page.Base_page):
    ...
    cgrk_cangku_guolv_cdyguangdong = (by.XPATH, "//*[@class='android.view.View'][@content-desc='cdy广东仓']")

    def click_cgrk_cangku_guolv_cdyguangdong(self):
        # 判断能否进行点击操作，如果能够点击操作，赋值1，同时输出点击成功的提示
        try:
            self.find_element (*self.cgrk_cangku_guolv_cdyguangdong).click ()
            self.dy_cgrk_cangku_guolv_cdyguangdong = 1
            print ("cdy广东点击成功")
        # 如果不能进行点击操作，赋值2，同时输出点击不成功的提示，并且输出当前页的截图
        except:
            self.dy_cgrk_cangku_guolv_cdyguangdong = 2
            print ("cdy广东点击失败，输出当前截图")
            self.driver.get_screenshot_as_file (
                "C:\\Users\\Administrator\\PycharmProjects\\pythonProject\\cqydtest\\src\\image\\click_cgrk_cangku_guolv_cdyguangdong.png")